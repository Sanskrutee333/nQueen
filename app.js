...
//THIS nQueenArr DEFINES THE NO OF SOLUTION FOR THE NUMBER OF QUEENS RANGING FROM 0 TO 8
let nQueenArr = [0, 2, 1, 1, 3, 11, 5, 41, 93];
...
class Queen {
    ...
    isValid = async (board, r, col, n) => {
        ...
        for (let i = r - 1; i >= 0; --i) {
            ...
            const value = column.innerHTML; if (value == queen) {
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#769656")
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "#ededd1");
        }
    }
...
playButton.onclick = async function visualise() {
    ...
    para.innerHTML = `For ${n}x${n} board with n Queens , ${nQueenArr[n] - 1} arrangements are possible: `;
    arrangement.appendChild(para);
    //FORMATION OF BOARDS
    if (chessBoard.childElementCount === 0) {
        for (let i = 0; i < nQueenArr[n]; ++i) {
    ...
    for (let k = 0; k < nQueenArr[n]; ++k) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                (i + j) & 1
                    ? (col.style.backgroundColor = "#769656")
                    : (col.style.backgroundColor = "#ededd1");
                col.innerHTML = "-";
